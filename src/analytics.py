...
def portfolio_pnl():
    ...
    for ticker, lots in holdings.items():
        ...
        if total_qty == 0:
            continue
        avg_buy_price = sum(lot['qty'] * lot['price'] for lot in lots) / total_qty
        current_price = get_price(ticker)
        if current_price is None:
            continue
        pnl_per_share = current_price - avg_buy_price
        total_pnl = pnl_per_share * total_qty
        total_portfolio_pnl += total_pnl
    return round(total_portfolio_pnl, 2)
...
def portfolio_risk_metrics():
    ...
    for ticker, lots in holdings.items():
        ...
        beta = lots[0].get('beta', 1) if 'beta' in lots[0] else 1
        betas.append(beta)
    ...
    if total_value > 0 and betas and weights:
        weighted_beta = round(np.average(betas, weights=weights), 2)
    else:
        weighted_beta = 1.0
